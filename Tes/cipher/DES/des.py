# cipher/DES/des.py
import os
from .utils import (
    bytes_to_bits, bits_to_bytes, permute, xor_bits, 
    left_shift, pkcs7_pad, pkcs7_unpad
)

# ===== DES TABLES =====

# Initial Permutation (IP)
IP = [
    58, 50, 42, 34, 26, 18, 10, 2,
    60, 52, 44, 36, 28, 20, 12, 4,
    62, 54, 46, 38, 30, 22, 14, 6,
    64, 56, 48, 40, 32, 24, 16, 8,
    57, 49, 41, 33, 25, 17, 9, 1,
    59, 51, 43, 35, 27, 19, 11, 3,
    61, 53, 45, 37, 29, 21, 13, 5,
    63, 55, 47, 39, 31, 23, 15, 7
]

# Final Permutation (IP^-1)
FP = [
    40, 8, 48, 16, 56, 24, 64, 32,
    39, 7, 47, 15, 55, 23, 63, 31,
    38, 6, 46, 14, 54, 22, 62, 30,
    37, 5, 45, 13, 53, 21, 61, 29,
    36, 4, 44, 12, 52, 20, 60, 28,
    35, 3, 43, 11, 51, 19, 59, 27,
    34, 2, 42, 10, 50, 18, 58, 26,
    33, 1, 41, 9, 49, 17, 57, 25
]

# Expansion (E)
E = [
    32, 1, 2, 3, 4, 5,
    4, 5, 6, 7, 8, 9,
    8, 9, 10, 11, 12, 13,
    12, 13, 14, 15, 16, 17,
    16, 17, 18, 19, 20, 21,
    20, 21, 22, 23, 24, 25,
    24, 25, 26, 27, 28, 29,
    28, 29, 30, 31, 32, 1
]

# Permutation (P)
P = [
    16, 7, 20, 21, 29, 12, 28, 17,
    1, 15, 23, 26, 5, 18, 31, 10,
    2, 8, 24, 14, 32, 27, 3, 9,
    19, 13, 30, 6, 22, 11, 4, 25
]

# Permuted Choice 1 (PC1)
PC1 = [
    57, 49, 41, 33, 25, 17, 9,
    1, 58, 50, 42, 34, 26, 18,
    10, 2, 59, 51, 43, 35, 27,
    19, 11, 3, 60, 52, 44, 36,
    63, 55, 47, 39, 31, 23, 15,
    7, 62, 54, 46, 38, 30, 22,
    14, 6, 61, 53, 45, 37, 29,
    21, 13, 5, 28, 20, 12, 4
]

# Permuted Choice 2 (PC2)
PC2 = [
    14, 17, 11, 24, 1, 5,
    3, 28, 15, 6, 21, 10,
    23, 19, 12, 4, 26, 8,
    16, 7, 27, 20, 13, 2,
    41, 52, 31, 37, 47, 55,
    30, 40, 51, 45, 33, 48,
    44, 49, 39, 56, 34, 53,
    46, 42, 50, 36, 29, 32
]

# S-Boxes
S_BOXES = [
    # S1
    [
        [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
        [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
        [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
        [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]
    ],
    # S2
    [
        [15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
        [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
        [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
        [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]
    ],
    # S3
    [
        [10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
        [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
        [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
        [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]
    ],
    # S4
    [
        [7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
        [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
        [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
        [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]
    ],
    # S5
    [
        [2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
        [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
        [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
        [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]
    ],
    # S6
    [
        [12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
        [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
        [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
        [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]
    ],
    # S7
    [
        [4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
        [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
        [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
        [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]
    ],
    # S8
    [
        [13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
        [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
        [7, 11, 4, 1, 9, 12, 14, 2, 0, 5, 10, 3, 13, 8, 15, 6],
        [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]
    ]
]

# Left shifts schedule
LEFT_SHIFTS = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]


# ===== HELPER FUNCTIONS =====

def s_box_lookup(block_6: list, s_box_index: int) -> list:
    """Tra cứu S-box"""
    row = (block_6[0] << 1) | block_6[5]
    col = (block_6[1] << 3) | (block_6[2] << 2) | (block_6[3] << 1) | block_6[4]
    val = S_BOXES[s_box_index][row][col]
    return [(val >> (3 - i)) & 1 for i in range(4)]


def f_function(right: list, subkey: list) -> list:
    """Hàm F của DES"""
    # Expansion
    expanded = permute(right, E)
    
    # XOR with subkey
    xored = xor_bits(expanded, subkey)
    
    # S-boxes
    output = []
    for i in range(8):
        block_6 = xored[i * 6:(i + 1) * 6]
        output.extend(s_box_lookup(block_6, i))
    
    # Permutation P
    return permute(output, P)


def generate_subkeys(key_64: list) -> list:
    """Sinh 16 subkey từ key 64-bit"""
    # PC1
    key_56 = permute(key_64, PC1)
    
    # Chia thành C và D
    C = key_56[:28]
    D = key_56[28:]
    
    subkeys = []
    for i in range(16):
        # Left shift
        C = left_shift(C, LEFT_SHIFTS[i])
        D = left_shift(D, LEFT_SHIFTS[i])
        
        # PC2
        subkey = permute(C + D, PC2)
        subkeys.append(subkey)
    
    return subkeys


def des_block(block_64: list, subkeys: list, decrypt=False) -> list:
    """Mã hóa/giải mã 1 block 64-bit"""
    if decrypt:
        subkeys = subkeys[::-1]
    
    # Initial Permutation
    block = permute(block_64, IP)
    
    # Chia L và R
    left = block[:32]
    right = block[32:]
    
    # 16 rounds
    for i in range(16):
        new_right = xor_bits(left, f_function(right, subkeys[i]))
        left = right
        right = new_right
    
    # Swap và Final Permutation
    final = permute(right + left, FP)
    return final


# ===== MAIN ENCRYPTION/DECRYPTION =====

def parse_key(key_str: str) -> bytes:
    """Parse key từ string"""
    k = key_str.strip()
    if len(k) == 16 and all(c in "0123456789abcdefABCDEF" for c in k):
        return bytes.fromhex(k)
    else:
        kb = k.encode()
        if len(kb) != 8:
            raise ValueError("DES key must be 8 bytes or 16 hex chars")
        return kb


def encrypt_bytes(plaintext: bytes, key: bytes) -> bytes:
    """Mã hóa DES (CBC mode)"""
    if len(key) != 8:
        raise ValueError("DES key must be 8 bytes")
    
    # Generate subkeys
    key_bits = bytes_to_bits(key)
    subkeys = generate_subkeys(key_bits)
    
    # Padding
    plaintext_padded = pkcs7_pad(plaintext, 8)
    
    # Random IV
    iv = os.urandom(8)
    iv_bits = bytes_to_bits(iv)
    
    # CBC mode
    prev = iv_bits
    ciphertext_bits = []
    
    for i in range(0, len(plaintext_padded), 8):
        block = plaintext_padded[i:i+8]
        block_bits = bytes_to_bits(block)
        
        # XOR with previous ciphertext
        xored = xor_bits(block_bits, prev)
        
        # Encrypt
        encrypted = des_block(xored, subkeys, decrypt=False)
        ciphertext_bits.extend(encrypted)
        
        prev = encrypted
    
    ciphertext = bits_to_bytes(ciphertext_bits)
    return iv + ciphertext


def decrypt_bytes(iv_and_ciphertext: bytes, key: bytes) -> bytes:
    """Giải mã DES (CBC mode)"""
    if len(key) != 8:
        raise ValueError("DES key must be 8 bytes")
    
    if len(iv_and_ciphertext) < 8:
        raise ValueError("Invalid ciphertext")
    
    # Generate subkeys
    key_bits = bytes_to_bits(key)
    subkeys = generate_subkeys(key_bits)
    
    # Extract IV
    iv = iv_and_ciphertext[:8]
    ciphertext = iv_and_ciphertext[8:]
    
    if len(ciphertext) % 8 != 0:
        raise ValueError("Invalid ciphertext length")
    
    # CBC mode
    iv_bits = bytes_to_bits(iv)
    prev = iv_bits
    plaintext_bits = []
    
    for i in range(0, len(ciphertext), 8):
        block = ciphertext[i:i+8]
        block_bits = bytes_to_bits(block)
        
        # Decrypt
        decrypted = des_block(block_bits, subkeys, decrypt=True)
        
        # XOR with previous ciphertext
        plain_block = xor_bits(decrypted, prev)
        plaintext_bits.extend(plain_block)
        
        prev = block_bits
    
    plaintext_padded = bits_to_bytes(plaintext_bits)
    return pkcs7_unpad(plaintext_padded)


# ===== CLASS WRAPPER =====

class DESCipher:
    def __init__(self):
        pass
    
    def encrypt_bytes_with_key(self, data: bytes, key_str: str) -> bytes:
        key = parse_key(key_str)
        return encrypt_bytes(data, key)
    
    def decrypt_bytes_with_key(self, data: bytes, key_str: str) -> bytes:
        key = parse_key(key_str)
        return decrypt_bytes(data, key)
    
    def encrypt_file(self, input_path: str, output_path: str, key_str: str):
        with open(input_path, "rb") as f:
            plain = f.read()
        enc = self.encrypt_bytes_with_key(plain, key_str)
        with open(output_path, "wb") as f:
            f.write(enc)
    
    def decrypt_file(self, input_path: str, output_path: str, key_str: str):
        with open(input_path, "rb") as f:
            data = f.read()
        plain = self.decrypt_bytes_with_key(data, key_str)
        with open(output_path, "wb") as f:
            f.write(plain)